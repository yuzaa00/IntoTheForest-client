    ...
    public create(): void {
      this.game.input.addPointer()
       ...
       this.stage1Bgm.sound.add('stage2-1_bgm').play({
        ...
        skyTile = this.add.tileSprite(0, 0, 30000, 600, 'stage2').setScrollFactor(0).setOrigin(0).setDepth(0)
        ...
        for (let i = 0; i < this.registry.values.bird; i++) {
          this.getSubcha(player, 'bird')
        }
        for (let i = 0; i < this.registry.values.squi; i++) {
          this.getSubcha(player, 'squi')
        }


        // while(this.registry.values.bird > 0 || this.registry.values.squi > 0) {
    ...
    private worldTime(): void {  // 1초당 실행되는 함수 this.worldTimer 참조
      ...
      collectSignExit(player: any, tile: any):void {
        ...
        if(sign.index !== -1) {
          ...
          this.scene.start('Stage2Event', { score: this.registry.values.score + 10000, life: this.registry.values.life + 1000, stage: 2  }) // stage1Event로 scene 이동 (데이터 이전) 
        }
      }
